...
{
    /// <summary>
    /// Person Allow Checkin By Transformation
    /// </summary>
    /// 
    [Description( "Transform result to Allow Checkin By" )]
    [Export( typeof( DataTransformComponent ) )]
    [ExportMetadata( "ComponentName", "Person Allow Checkin By Transformation" )]
    public class AllowCheckInByTransform : DataTransformComponent<Rock.Model.Person>
    {
        /// <summary>
        /// Gets the title.
        /// </summary>
        ...
        private Expression BuildExpression( IService serviceInstance, IQueryable<int> idQuery, ParameterExpression parameterExpression )
        {
            ...
            int allowCheckInByRoleId = groupeType.Roles.Where( a => a.Guid == Rock.SystemGuid.GroupRole.GROUPROLE_KNOWN_RELATIONSHIPS_ALLOW_CHECK_IN_BY.AsGuid() )
                .Select( a => a.Id )
                .FirstOrDefault();

            var qry = new PersonService( ( RockContext ) serviceInstance.Context ).Queryable()
                .Where( p => p.Members.Where( a => a.GroupRoleId == allowCheckInByRoleId )

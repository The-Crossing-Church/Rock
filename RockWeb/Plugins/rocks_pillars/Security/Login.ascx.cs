...
namespace RockWeb.Plugins.rocks_pillars.Security
{
    ...
    /// </summary>
    /// <remarks>
    /// This is a copy of the core v13.4 block with following changes:
    /// - Update how block handles authentication providers with "Remote Authentication Required" flag. This is to support
        ...
        private void ApplyBlockSettings()
        {
            ...
            foreach ( var serviceEntry in AuthenticationContainer.Instance.Components )
            {
                    ...
                    selectedGuids.Contains( component.EntityType.Guid ) )
                {
                    ...
                    // CUSTOM CHANGE: 
                    // The core block attempts to add a linkbutton for any component that is marked with the "Requires Remote Authentication". This 
                    // "If" statement below was added so that the block only adds a linkbutton if the component is also marked as an "External" component.  
                    // All of the existing remote authentication components (Facebook, Twitter, Google) have both settings so they will still add
                    // the link button. The new 2FA is the only component that is marked as "Requiring Remote Authentication", but not "External."
                    // Because of that, it is the only compoenent that will reach this code, and not match the following IF statement. 
                    if ( component.ServiceType == AuthenticationServiceType.External )
                    {
        ...
        protected void btnLogin_Click( object sender, EventArgs e )
        {
            if ( Page.IsValid )
            {
                ...
                if ( userLogin != null && userLogin.EntityType != null )
                {
                    ...
                    // CUSTOM CHANGE: 
                    // The following code within this If statement was added. The core block does not allow a component marked as 
                    // "Requires Remote Authentication" to use the normal username/password and login buttons. However, because
                    // a LinkButton is now only used if the "External" flag is true, any Remote Authentication components that are
                    // not "External", will now use the Login button. This code was added to handle that scenario and redirect user
                    // to the remote authentication url.
                    if ( component != null && component.IsActive && component.RequiresRemoteAuthentication )
                    {

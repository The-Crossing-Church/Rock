...
namespace RockWeb.Plugins.rocks_pillars.Finance
{
    ...
    public partial class TransactionYearlySummaryLava : RockBlock, ISecondaryBlock
    {
        ...
        private class SummaryRecord
        {
            ...
            public decimal SoftCreditAmount { get; set; }
        }
            ...
            if ( contributionType != null )
            {
                ...
                if ( targetPerson != null )
                {
                    ...
                    if ( txnDetailEntityType != null )
                    {
                        var personAliasGuids = new PersonAliasService( rockContext ).Queryable().AsNoTracking()
                            .Where( pa => pa.Person.GivingLeaderId == targetPerson.GivingLeaderId )
                            .Select( pa => pa.Guid )
                            .ToList()
                            .Select( g => g.ToString() )
                            .ToList();
                        ...
                        qry = qry.Where( t => 
                            t.Transaction.AuthorizedPersonAlias.Person.GivingLeaderId == targetPerson.GivingLeaderId ||
                            softCreditTxnDetailIdQry.Contains( t.Id ) );
                    }
                    else
                    {
                        qry = qry.Where( t => t.Transaction.AuthorizedPersonAlias.Person.GivingLeaderId == targetPerson.GivingLeaderId );
                    }
                }
                ...
                using ( new Rock.Data.QueryHintScope( rockContext, QueryHintType.RECOMPILE ) )
                {
                    summaryList = qry
                        .Select( a => new
                        {
                            a.Transaction.TransactionDateTime.Value.Year,
                            a.AccountId,
                            a.Amount,
                            a.Transaction.AuthorizedPersonAlias.Person.GivingLeaderId
                        } )
                        .ToList()
                        ...
                        .Select( t => new SummaryRecord
                        {
                            ...
                            SoftCreditAmount = t.Sum( d => d.GivingLeaderId == targetPerson.GivingLeaderId ? 0 : d.Amount )
                ...
                foreach ( var item in summaryList.GroupBy( a => a.Year ) )
                {
                    ...
                    foreach ( var a in item )
                    {
                        ...
                        accountDictionary.Add( "SoftCreditAmount", a.SoftCreditAmount );

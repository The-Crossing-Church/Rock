...
namespace Plugins.com_simpledonation.Event
{
    /// <summary>
    /// Block used to register for a registration instance.
    /// </summary>
    [DisplayName( "Simple Donation Registration Entry" )]
    ...
    [BooleanField( "Enable ACH", "If true, Event Registrations can be paid via ACH", false, "Simple Donation", 15, IsRequired = true )]
    [BooleanField( "ACH Default", "If true, ACH will automatically be selected as the default payment type", false, "Simple Donation", 16, IsRequired = true )]
    [BooleanField( "Show Cover Fees", "If true, this will show the button enabling fees to be covered by the giver.", false, "Simple Donation", 17, IsRequired = true )]
    [BooleanField( "Cover Fees By Default", "If true, the checkbox for cover fees will be checked by default", false, "Simple Donation", 18, IsRequired = true )]
    [AccountField( "Cover Fee Account", "An optional account for cover fees to be saved to.", false, "", "Simple Donation", 19 )]
    ...
    public partial class RegistrationEntry : RockBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                else
                {
                    ...
                    if ( RegistrationState != null || SetRegistrationState() )
                    {
                        ...
                        {
                            /* SimpleDonation.Start */
                            RegisterFeeRates( RegistrationTemplate.FinancialGatewayId );
                            /* SimpleDonation.End */
            ...
            else
            {
                ...
                divStrike.Visible = ( rblSavedCC.Items.Count == 0 || rblSavedCC.Items[rblSavedCC.Items.Count - 1].Selected ) ? true : false;
                divACHPaymentInfo.Style[HtmlTextWriterStyle.Display] = ( rblSavedCC.Items.Count == 0 || rblSavedCC.Items[rblSavedCC.Items.Count - 1].Selected ) ? "block" : "none";
                btnCreditCard.Visible = ( rblSavedCC.Items.Count == 0 || rblSavedCC.Items[rblSavedCC.Items.Count - 1].Selected ) ? true : false;
                btnACH.Visible = ( rblSavedCC.Items.Count == 0 || rblSavedCC.Items[rblSavedCC.Items.Count - 1].Selected ) ? true : false;
        ...
        protected void sm_Navigate( object sender, HistoryEventArgs e )
        {
            ...
            /* SimpleDonation.Start */
            /*
             * This solves an issue with Apple Pay where the Success panel throws an exception when trying to save itself in the base.OnPreRender method, so the last stored
             * event in memory is that the current panel is the Summary panel. Based on the comment in the OnPreRender method, this is failing because we're programmatically
             * Calling the next page button click instead of really clicking it.
            */
            if ( hfPostbackFromModal.Value.IsNotNullOrWhiteSpace() && hfPostbackFromModal.Value == "True" )
            {
                hfPostbackFromModal.Value = "False";
                nbMain.Title = "Please correct the following:";
                nbMain.Visible = true;
            }
            /* SimpleDonation.End */
            else if ( CurrentPanel > 0 && state != null && hfAllowNavigate.Value.AsBoolean() )
            {
        ...
        protected void lbSummaryNext_Click( object sender, EventArgs e )
        {
            if ( CurrentPanel == PanelIndex.PanelSummary )
            {
                ...
                if ( !summaryErrors.Any() )
                {
                    ...
                    {
                        /* SimpleDonation.Start */
                        // Checking if we need to include the fees
                        if ( cbCoverFees.Checked )
                        {
                            nbAmountPaid.Text = ( nbAmountPaid.Text.AsDouble() + hfFeeAmount.Value.AsDouble() ).FormatAsCurrency();
                        }
                        /* SimpleDonation.End */
        ...
        private List<string> ValidateSummary()
        {
            ...
            if ( minimumPayment.HasValue && minimumPayment > 0.0M || RegistrationState.PaymentAmount > 0.0M )
            {
                ...
                if ( !Using3StepGateway && ( rblSavedCC.Items.Count == 0 || ( rblSavedCC.SelectedValueAsInt() ?? 0 ) == 0 ) )
                {
                    ...
                    /* SimpleDonation.Start */
                    AddAchValidationErrors( validationErrors );
                    /* SimpleDonation.End */
                }

                /* SimpleDonation.Start */
                AddStripeTokenValidationErrors( validationErrors );
                /* SimpleDonation.End */
        ...
        private bool ProcessPayment( RockContext rockContext, Registration registration, out string errorMessage )
        {
            ...
            if ( rblSavedCC.Items.Count > 0 && ( rblSavedCC.SelectedValueAsId() ?? 0 ) > 0 )
            {
                ...
                if ( savedAccount != null )
                {
                    ...
                    referencePaymentInfo.Amount = RegistrationState.PaymentAmount ?? 0.0m;
                    /* SimpleDonation.Start */
                    referencePaymentInfo.Amount = nbAmountPaid.Text.AsDecimal();
            ...
            else
            {
                ...
                /* SimpleDonation.Start */
                AddAchErrorMessages( ref isValid, ref errorMessages );
                AddStripeTokenErrorMessages( ref isValid, ref errorMessages );
                /* SimpleDonation.End */
                ...
                /* SimpleDonation.Start */
                paymentInfo = GetPaymentInfo( gateway );
        ...
        private bool SaveTransaction( GatewayComponent gateway, Registration registration, FinancialTransaction transaction, PaymentInfo paymentInfo, RockContext rockContext )
        {
            if ( transaction != null )
            {
                ...
                var feeAccountGuid = GetAttributeValue( "CoverFeeAccount" ).AsGuidOrNull();
                ...
                /* SimpleDonation.Start */
                if ( feeAccount != null && feeAccount.Id != RegistrationInstanceState.AccountId.Value && cbCoverFees.Checked )
                {
                    transactionDetail.Amount = nbAmountPaid.Text.AsDecimal() - hfFeeAmount.Value.AsDecimal();
                }
                else
                {
                ...
                if ( feeAccount != null && feeAccount.Id != RegistrationInstanceState.AccountId.Value && cbCoverFees.Checked )
                {
                    ...
                    feeTransactionDetail.Amount = hfFeeAmount.Value.AsDecimal();
        ...
        private bool ProcessStep1( out string errorMessage )
        {
            ...
            if ( rblSavedCC.Items.Count > 0 && ( rblSavedCC.SelectedValueAsId() ?? 0 ) > 0 )
            {
                ...
                if ( savedAccount != null )
                {
                    ...
                    /* SimpleDonation.Start */
                    paymentInfo.Amount = nbAmountPaid.Text.AsDecimal();
                    /* SimpleDonation.End */
                }
                else
                {
        ...
        private CreditCardPaymentInfo GetCCPaymentInfo( GatewayComponent gateway )
        {
            ...
            /* SimpleDonation.Start */
            ccPaymentInfo.Amount = nbAmountPaid.Text.AsDecimal();
            /* SimpleDonation.End */
        ...
        private bool ShowInstructions()
        {
            ...
            /* SimpleDonation.Start */
            if ( !string.IsNullOrEmpty( instructions.SanitizeHtml() ) || instructions.Contains( "<img" ) )
            {
                lInstructions.Text = string.Format( "<div class='text-left'>{0}</div>", instructions );
            }

            return instructions.SanitizeHtml().IsNotNullOrWhiteSpace() || instructions.Contains( "<img" );
            /* SimpleDonation.End */
        }
        ...
        private void RegisterClientScript()
        {
            ...
            /* SimpleDonation.Start */
            RockPage.AddScriptLink( "https://js.stripe.com/v3/", false );
            RockPage.AddScriptLink( "~/Plugins/com_simpledonation/js/cover-fee-reg.js" );
            RockPage.AddScriptLink( "~/Plugins/com_simpledonation/js/payment-request-button.js" );
            /* SimpleDonation.End */
...
",
        ...
        private void CreateSummaryControls( bool setValues )
        {
            ...
            if ( setValues && RegistrationState != null && RegistrationInstanceState != null )
            {
                     ...
                     RegistrationTemplate.RegistrarOption == RegistrarOption.PrefillFirstRegistrant )
                {
                    ...
                    {
                        pnlRegistrarInfoPrompt.Style[HtmlTextWriterStyle.Display] = "none";
                    }
                ...
                if ( costs.Where( c => c.Cost > 0.0M ).Any() )
                {
                    ...
                    /* SimpleDonation.Start */
                    if ( lRemainingDue.Text.AsDecimal() < 0 )
                    {
                        lRemainingDue.Text = 0.FormatAsCurrency();
                    }
                    ProcessBlockSettings( true );
                    /* SimpleDonation.End */
                    ...
                    if ( balanceDue > 0 && RegistrationTemplate.FinancialGateway != null )
                    {
                        ...
                        if ( component != null )
                        {
                            ...
                            else
                            {
                                divNewCard.Visible = true;
                                /* SimpleDonation.Start */
                                // Showing write payment type
                                if ( GetAttributeValue( "EnableACH" ).AsBoolean() && GetAttributeValue( "ACHDefault" ).AsBoolean() )
                                {
        ...
        private void BindSavedAccounts( GatewayComponent component )
        {
            ...
            if ( CurrentPerson != null )
            {
                ...
                if ( GetAttributeValue( "EnableACH" ).AsBoolean() )
                {
                    // Find the saved accounts that are valid for the selected ACH gateway
                    var achCurrencyType = DefinedValueCache.Get( new Guid( Rock.SystemGuid.DefinedValue.CURRENCY_TYPE_ACH ) );
                    if ( component != null &&
                        component.SupportsSavedAccount( false ) &&
                        component.SupportsSavedAccount( achCurrencyType ) )
                    {
                ...
                // Bind the accounts
                rblSavedCC.DataSource = savedAccountsById;
                rblSavedCC.DataBind();
                if ( rblSavedCC.Items.Count > 0 )
                {
                    ...
                    if ( rblSavedCC.Items.Count > 0 )
                    {
                        rblSavedCC.Items.Add( new ListItem( "Use a different payment method", "0" ) );
        ...
        public static class AttributeKeys
        {
            ...
            public const string ACHDefault = "ACHDefault";
            public const string ShowCoverFees = "ShowCoverFees";
            public const string CoverFeesByDefault = "CoverFeesByDefault";
        }

        #region SimpleDonation

        #region Events

        /// <summary>
        /// Handles the Click event of the btnCreditCard control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnCreditCard_Click( object sender, EventArgs e )
        {
            SetPaymentType( CurrencyType.Card );
        }

        /// <summary>
        /// Handles the Click event of the btnACH control.
        /// </summary>
        /// <param name="sender">The source of the event.</param>
        /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
        protected void btnACH_Click( object sender, EventArgs e )
        {
            SetPaymentType( CurrencyType.ACH );
        }
        ...
        /// <summary>
        /// Based on the type passed in, this hides the correct fields
        /// </summary>
        /// <param name="currencyType">Type of the currency.</param>
        private void SetPaymentType( CurrencyType currencyType )
        {
            switch ( currencyType )
            {
                case CurrencyType.ACH:
                    divACHPaymentInfo.Visible = true;
                    divNewCard.Visible = false;
        ...
        /// <summary>
        /// Registers the fee rates.
        /// This updated the hidden fields with the correct information so JS can access them
        /// </summary>
        private void RegisterFeeRates( int? financialGatewayId )
        {
            ...
            if ( accountInfo.PublicKey.IsNullOrWhiteSpace() )
            {
        ...
        private PaymentInfo GetPaymentInfo( GatewayComponent gateway )
        {
            ...
            Location homeLocation = CurrentPerson.GetHomeLocation();

            if ( hfStripeToken.Value.IsNotNullOrWhiteSpace() && hfStripeToken.Value.StartsWith( "tok" ) )
            {
                var currencyTypeGuid = Rock.SystemGuid.DefinedValue.CURRENCY_TYPE_CREDIT_CARD.AsGuid(); // Set backup value

                var walletName = hfWalletName.Value ?? "";
                if ( walletName == "applePay" )
                {
                    currencyTypeGuid = Rock.SystemGuid.DefinedValue.CURRENCY_TYPE_APPLE_PAY.AsGuid();
                }
                if ( walletName == "googlePay" )
                {
                    currencyTypeGuid = Rock.SystemGuid.DefinedValue.CURRENCY_TYPE_ANDROID_PAY.AsGuid();
                }
            ...
            else if ( hfPaymentTab.Value == "ACH" )
            {
                paymentInfo = GetACHInfo();
            }
            else
            {
                paymentInfo = GetCCInfo( gateway );
            }
        ...
        /// </summary>
        /// <returns></returns>
        private CreditCardPaymentInfo GetCCInfo( GatewayComponent gateway )
        {
            var ccPaymentInfo = new CreditCardPaymentInfo( txtCreditCard.Text, txtCVV.Text, mypExpiration.SelectedDate != null ? mypExpiration.SelectedDate.Value : new DateTime() );
            ...
            var simpleDonationCcPaymentInfo = new SimpleDonationCreditCardPaymentInfo( cbCoverFees.Checked, ccPaymentInfo );
            return simpleDonationCcPaymentInfo;
        }
        ...
        {
            return new SimpleDonationACHPaymentInfo( cbCoverFees.Checked, new ACHPaymentInfo( txtAccountNumber.Text, txtRoutingNumber.Text, rblAccountType.SelectedValue == "Savings" ? BankAccountType.Savings : BankAccountType.Checking ) );
        }

        /// <summary>
        /// Adds the ach error messages.
        /// </summary>
        /// <param name="isValid">if set to <c>true</c> [is valid].</param>
        /// <param name="errorMessages">The error messages.</param>
        private void AddAchErrorMessages( ref bool isValid, ref List<string> errorMessages )
        {
            // Checking if Card of ACH
        ...
        /// <summary>
        /// Adds the stripe token error messages.
        /// </summary>
        /// <param name="isValid">if set to <c>true</c> [is valid].</param>
        /// <param name="errorMessages">The error messages.</param>
        private void AddStripeTokenErrorMessages( ref bool isValid, ref List<string> errorMessages )
        {
            // Checking if stripe
            if ( hfStripeToken.Value.IsNotNullOrWhiteSpace() && hfStripeToken.Value.StartsWith( "tok" ) )
            {
                var walletName = hfWalletName.Value ?? "";
                if ( walletName == "applePay" || walletName == "googlePay" )
                {
                    isValid = true;
                    errorMessages = new List<string>();
                    ...
                    if ( string.IsNullOrWhiteSpace( tbYourLastName.Text ) )
                    {
        ...
        /// <summary>
        /// Adds the ach validation errors.
        /// </summary>
        /// <param name="validationErrors">The validation errors.</param>
        private void AddAchValidationErrors( List<string> validationErrors )
        {
            // Checking if we are looking at ACH or Card
            if ( hfPaymentTab.Value == "ACH" && ( hfWalletName == null || hfWalletName.Value.IsNullOrWhiteSpace() || hfWalletName.Value != "applePay" || hfWalletName.Value != "googlePay" ) )
            {
                // validate ach options
                if ( string.IsNullOrWhiteSpace( txtRoutingNumber.Text ) )
                {
                    validationErrors.Add( "Make sure to enter a valid routing number" );
                }

                if ( string.IsNullOrWhiteSpace( txtAccountNumber.Text ) )
                {
                    validationErrors.Add( "Make sure to enter a valid account number" );
                }

                validationErrors.RemoveAll( r => r == "First Name on Card is required" );
                validationErrors.RemoveAll( r => r == "Last Name on Card is required" );
                validationErrors.RemoveAll( r => r == "Name on Card is required" );
                validationErrors.RemoveAll( r => r == "Card Number is required" );
                validationErrors.RemoveAll( r => r == "Card Expiration Date is required" );
                validationErrors.RemoveAll( r => r == "Card Security Code is required" );
                validationErrors.RemoveAll( r => r == "Billing Address is required" );
        ...
        /// </summary>
        /// <param name="validationErrors">The validation errors.</param>
        private void AddStripeTokenValidationErrors( List<string> validationErrors )
        {
            // Checking if stripe
            if ( hfStripeToken.Value.IsNotNullOrWhiteSpace() && hfStripeToken.Value.StartsWith( "tok" ) )
            {
                ...
                if ( walletName == "applePay" || walletName == "googlePay" )
                {
                    ...
                    validationErrors.RemoveAll( r => r == "Make sure to enter a valid routing number" );
                    validationErrors.RemoveAll( r => r == "Make sure to enter a valid account number" );
                    validationErrors.RemoveAll( r => r == "First Name on Card is required" );
                    validationErrors.RemoveAll( r => r == "Last Name on Card is required" );
                    validationErrors.RemoveAll( r => r == "Name on Card is required" );
                    validationErrors.RemoveAll( r => r == "Card Number is required" );
        ...
        /// <summary>
        /// Processes the block settings.
        /// </summary>
        private void ProcessBlockSettings( bool setValues = false )
        {
            // Checking if ACH is enabled for this block
            if ( GetAttributeValue( "EnableACH" ).AsBoolean() )
            {
                if ( GetAttributeValue( "ACHDefault" ).AsBoolean() )
                {
                    SetPaymentType( CurrencyType.ACH );
                }
                else
            ...
            {
                SetPaymentType( CurrencyType.Card );
                // If we only offer credit card support, hide both buttons
                liACH.Visible = false;
                liCreditCard.Visible = false;
            }

            // Checking if Cover fees should be displayed
            if ( GetAttributeValue( "ShowCoverFees" ).AsBoolean() )
            {
                cbCoverFees.Visible = true;
                liCoverFeesParent.Visible = true;
                cbCoverFees.Label = "Cover the $0 in processing fees";

                if ( setValues )
                {
                    if ( GetAttributeValue( "CoverFeesByDefault" ).AsBoolean() )
                    {
                        cbCoverFees.Checked = true;
                    }
                    else
                    {
                        cbCoverFees.Checked = false;
                    }
                }
            }
            else
            {
                liCoverFeesParent.Visible = false;
            }
        }
        ...
        /// <summary>
        /// Currency Type used throughout the application
        /// </summary>

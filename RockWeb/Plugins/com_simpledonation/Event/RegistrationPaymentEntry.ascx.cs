...
namespace RockWeb.Plugins.com_simpledonation.Event
{
    ...
    public partial class RegistrationPaymentEntry : Rock.Web.UI.RockBlock
    {
        ...
        {
            var rockContext = new RockContext(); var registration = GetRegistration();
            if ( registration != null )
            {
                    ...
                    if ( gatewayComponent != null )
                    {
                        if ( string.IsNullOrWhiteSpace( TransactionCode ) )
                        {
                            ...
                            if ( schedule != null )
                            {
                                ...
                                SaveScheduledTransaction( financialGateway, gatewayComponent, person, paymentInfo, schedule, scheduledTransaction, rockContext );
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    SaveTransaction( financialGateway, gatewayComponent, person, paymentInfo, transaction, rockContext );
        ...
        private void SetScheduleOptions()
        {
            ...
            if ( !string.IsNullOrWhiteSpace( PageParameter( AttributeKey.FrequencyParameter ) ) )
            {
                var frequencyValues = PageParameter( AttributeKey.FrequencyParameter ).Split( new char[] { '^' } );

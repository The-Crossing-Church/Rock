...
namespace Plugins.com_simpledonation
{
    ...
    [PersonField("Anonymous Person", "Person in the database to assign anonymous giving to.", true, SystemGuid.SIMPLE_DONATION_GIVER_PERSON_ALIAS, "", 3)]
    ...
    [BooleanField( "Cover Fees Checked", "Determines whether the 'cover fees' checkbox is checked by default", false, "", 13, IsRequired = true )]
    [BooleanField( "Cover Fees Visible", "Determines whether the 'cover fees' checkbox will be displayed", true, "", 14, IsRequired = true )]
    #endregion
    ...
    public partial class TransactionEntryKiosk : Rock.Web.UI.RockBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            RockPage.AddScriptLink( "~/Plugins/com_simpledonation/js/jquery.cardswipe.min.js" );
            RockPage.AddScriptLink( "~/Plugins/com_simpledonation/js/stripe-token-kiosk.js" );

            if ( _coverFeesVisible )
            {
                RockPage.AddScriptLink( "~/Plugins/com_simpledonation/js/cover-fees.js" );
            }
        }
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            _coverFeesVisible = Convert.ToBoolean( GetAttributeValue( "CoverFeesVisible" ) );
            ...
            if ( !Page.IsPostBack )
            {
                ...
                RegisterAccountInfo( financialGateway.Id);
        ...
        protected void lbAccountEntryNext_Click( object sender, EventArgs e )
        {
            ...
            {
                RockTextBox cb = phAccounts.FindControl( "tbAccount_" + account.Key.ToString() ) as RockTextBox;
                if ( cb != null )
                {
        ...
        private void ProcessSwipe( string swipeData )
        {
            try
            {
                ...
                {
                    dynamic swipeJson = JsonConvert.DeserializeObject( swipeData );
                    SimpleDonationPaymentInfo paymentInfo = new SimpleDonationPaymentInfo(
                        swipeJson.token != null ? swipeJson.token.ToString() : null,
                        swipeJson.firstName != null ? swipeJson.firstName.ToString() : null,
                        swipeJson.lastName != null ? swipeJson.lastName.ToString() : null,
                        cbCoverFees.Checked );
                    paymentInfo.Amount = this.Amounts.Sum( a => a.Value );
                    decimal feeAmount;
                    FeeCalculationResult feeResult = null;

                    if ( cbCoverFees.Checked && decimal.TryParse( hfFeeAmount.Value, out feeAmount ) )
                    {
                        feeResult = _feeCalculationService.CalculateFees( Amounts, feeAmount );
                        paymentInfo.Amount = paymentInfo.Amount + feeResult.Total;
                    }

                    PersonAlias giver = new PersonAliasService( rockContext ).Queryable( "Person, Person.PhoneNumbers" ).Where( p => p.Id == this.SelectedGivingUnit.PersonAliasId ).FirstOrDefault();
                    paymentInfo.FirstName = giver.Person.NickName;
                    paymentInfo.LastName = giver.Person.LastName;
                    paymentInfo.Email = giver.Person.Email;
                    ...
                    if ( this.AnonymousGiverPersonAliasId != this.SelectedGivingUnit.PersonAliasId )
                    {
                        ...
                        if ( homeLocation != null )
                        {
                            ...
                            if ( !string.IsNullOrWhiteSpace( homeLocation.Country ) )
                            {
                                paymentInfo.Country = homeLocation.Country;
                            }
                            else
                            {
                                paymentInfo.Country = GlobalAttributesCache.Get().OrganizationCountry;
                            }
                        }
                    ...
                    if ( gateway != null )
                    {
                        ...
                        if ( transaction != null )
                        {
                            ...
                            foreach ( var accountAmount in this.Amounts.Where( a => a.Value > 0 ) )
                            {
                                ...
                                if ( feeResult != null )
                                {
                                    Fee fee = feeResult.Fees.FirstOrDefault( f => f.Id == accountAmount.Key );
        ...
        {            
            ...
            mergeFields.Add( "ReceiptEmailed", "False" );
        ...
        private void RegisterAccountInfo(int? financialGatewayId)
        {
            const string accountInfoKey = "com.SimpleDonation.FeeRates";

...
namespace Plugins.com_simpledonation
{
        ...
        SimpleDonationMerlin : Rock.Web.UI.RockBlock
    {
		...
		protected void MerlinSettings()
            ...
            string AutoLaunchChoice = this.GetAttributeValue( "AutoLaunch" ); // Retrieves the setting for the Auto Launch Merlin choice
            string RockMobileMerlinAdjSetting = this.GetAttributeValue( "RockMobileMerlinAdj" ); // Retrieves the setting for the Rock Mobile Merlin user value
            ...
            string AutoLaunchYes = string.Concat( "<script type='text/javascript'> window.simpleDonationMerlinSettings = { open: true }; </script>" ); // autolaunches Merlin on page load
            string AutoLaunchNo = string.Concat( "<script type='text/javascript'> window.simpleDonationMerlinSettings = { open: false }; </script>" ); // no autolaunch
            string RockMobileValue = string.Concat( "&rockMobilePx=", RockMobileMerlinAdjSetting );

            if ( AutoLaunchChoice == "True" )
            {
                merlinAutoLaunch.Text = AutoLaunchYes; // sends final constructed autolaunch script to webform
            }

            else
            {
                merlinAutoLaunch.Text = AutoLaunchNo;
            }

            if ( RockMobileMerlinAdjSetting == "0" )
            {
                string NotMobile = null; // nothing to add to the script
            }

            else
            {
                string IsMobile = RockMobileValue; // sends final constructed RockMobilePx script to webform
            }


            // Constructs the Merlin widget script html for button
            string MerlinWidgetFull = string.Concat( "<script async src='https://merlin.simpledonation.com/js/installScript.js?key=", MerlinWidgetId , "&primaryColor=", PrimaryColorTrim , RockMobileValue , "'></script><button class='open-merlin'>", MerlinButtonText, "</button>" );

...
namespace RockWeb.Plugins.com_bemaservices.Core
{
    ...
    /// </summary>
    [DisplayName( "Custom History Log" )]
    [Category( "BEMA Services" )]
    ...
    [CategoryField( "History Categories","List of categories to include in the list.",true, "Rock.Model.History",order:1,required:false )]
    public partial class CustomHistoryLog : RockBlock, ISecondaryBlock
    {
        ...
        private void BindGrid()
        {
            if ( _entity != null )
            {
                ...
                if ( entityTypeCache != null )
                {
                    ...
                    var blockCategories = GetAttributeValue ( "HistoryCategories" ).SplitDelimitedValues ().AsGuidList ();
                    int? categoryId = gfSettings.GetUserPreference ( "Category" ).AsIntegerOrNull ();

                    if (blockCategories.Any())
                    {
                        if ( categoryId.HasValue )
                        {
                            historyCategories = historyCategories.Where ( a => a.Id == categoryId.Value );
                        }
                        else
                        {
                            historyCategories = historyCategories.Where ( a => blockCategories.Contains ( a.Guid ) );
                        }
        ...
        private void AddCorrectedColumn()
        {
            LinkButtonField btnCorrected = new LinkButtonField ();
            btnCorrected.ItemStyle.HorizontalAlign = HorizontalAlign.Center;
            btnCorrected.HeaderStyle.CssClass = "grid-columncommand";
            btnCorrected.ItemStyle.CssClass = "grid-columncommand";
            btnCorrected.Text = "<i class='fa fa-check'></i>";
            btnCorrected.CssClass = "btn btn-default btn-sm";
            btnCorrected.HeaderText = "Corrected";
            btnCorrected.ToolTip = "Mark As Corrected";
            btnCorrected.Click += btnCorrected_Click;
            btnCorrected.Visible = true;

            gHistory.Columns.Add ( btnCorrected );
        }

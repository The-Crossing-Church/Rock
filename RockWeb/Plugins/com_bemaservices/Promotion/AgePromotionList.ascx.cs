            ...
            var importTask = new Task ( () =>
            {
                ...
                if ( groupsSelected.Any () )
                {
                    ...
                    foreach ( var group in groupsToUpdate )
                    {
                        ...
                        if (shouldMatch && matchSize.Value > 0)
                        {
                            if ( GetAttributeValue ( "PromoteInactiveMembers" ).AsBoolean () )
                            {
                                qry = string.Format ( "Insert Into AttributeValue (IsSystem, EntityId, AttributeId, Value, Guid, CreatedDateTime, ModifiedDateTime, CreatedByPersonAliasId, ModifiedByPersonAliasId) " +
                                    "Select 0, gm.Id, {0}, Min(B.Guid) As Guid, NewId(), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, {2}, {2} From GroupMember gm" +
                                    ...
                                    " Outer Apply (" +
                                        ...
                                        " And g.Name <> B.Name Group By gm.Id",
                        ...
                        {
                            WriteProgressMessage ( "<div class='alert alert-warning margin-t-md'><strong>Error Inserting Attributes.</strong></div>", ex.Message );
                            return;
                        }
                    }
                ...
                .Select ( g => new GroupListRowInfo
                {
                    ...
                    StartDate = new Func<String> ( () =>
                    {
                        ...
                        if ( gr != null )
                        {
                            ...
                            {
                                return "ERROR - Start Date not valid.";
                            }
                        }
                        return String.Empty;
                    } ) (),
                    EndDate = new Func<String> ( () =>
                    {
                        ...
                        if ( gr != null )
                        {
                            ...
                            {
                                return "ERROR - End Date not valid.";
                            }
                        }
                        return String.Empty;
                    } ) ()

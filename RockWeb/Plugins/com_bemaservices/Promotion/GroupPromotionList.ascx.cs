...
namespace RockWeb.com_bemaservices.Promotion
{
    [DisplayName ( "Group Promotion List" )]
    [Category ( "BEMA Services > Promotion" )]
    ...
    [ContextAware]
    public partial class GroupPromotionList : RockBlock, ICustomGridColumns
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            if ( !Page.IsPostBack )
            {
                ...
                var qryAttributes = attributeService.Queryable ().Where ( a => a.Key == _groupKey ).Select ( a => a.Id ).ToList ();
                if ( qryAttributes.Count () == 0 )
                {
                    ...
                    newAttribute.Name = _groupName;
                    newAttribute.Key = _groupKey;
                    newAttribute.Guid = _groupAttributeGuid;
        ...
        protected void btnPromote_Click( object sender, EventArgs e )
        {
            ...
            var importTask = new Task ( () =>
            {
                ...
                var groupPromotionAttribute = AttributeCache.Get ( _groupAttributeGuid );
                ...
                if ( groupsSelected.Any () )
                {
                    ...
                    foreach ( var group in groupsToUpdate )
                    {
                        ...
                        var promotesToValue = group.GetAttributeValue ( _groupKey );
                        ...
                        if ( GetAttributeValue ( "PromoteInactiveMembers" ).AsBoolean () )
                        {
                            qry = string.Format ( "Insert Into AttributeValue (IsSystem, EntityId, AttributeId, Value, Guid, CreatedDateTime, ModifiedDateTime, CreatedByPersonAliasId, ModifiedByPersonAliasId) " +
                                "Select 0, Id, {0}, '{1}', NewId(), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, {3}, {3} From GroupMember Where GroupId = {2}",
                                memberAttributeId, promotesToValue, group.Id, this.CurrentPersonAliasId );
                        ...
                        {
                            qry = string.Format ( "Insert Into AttributeValue (IsSystem, EntityId, AttributeId, Value, Guid, CreatedDateTime, ModifiedDateTime, CreatedByPersonAliasId, ModifiedByPersonAliasId) " +
                                "Select 0, Id, {0}, '{1}', NewId(), CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, {3}, {3} From GroupMember Where GroupId = {2} and GroupMemberStatus = 1",
                                memberAttributeId, promotesToValue, group.Id, this.CurrentPersonAliasId );
                            rockContext.Database.ExecuteSqlCommand ( qry, new SqlParameter ( "@groupId", group.Id ) );
                        }

                        // Handle if undo attribute is not on destination group type (which doesn't have to be a check-in group)
                        var newGroup = groupService.Get ( promotesToValue.AsGuid() );

                        var qryAttributes = attributeService.Queryable ().Where ( a => a.Key == _undoKey 
                          && (a.EntityTypeQualifierValue == newGroup.GroupTypeId.ToString() || ( a.EntityTypeQualifierValue == newGroup.GroupType.InheritedGroupTypeId.ToString () ) ) )
                        ...
                        if ( qryAttributes.Count () == 0 )
                        {
                            // Create attribute
                            var newAttribute = new Rock.Model.Attribute ();
                            newAttribute.Name = _undoName;
                            newAttribute.Key = _undoKey;
                            newAttribute.Guid = Guid.NewGuid();
                            newAttribute.EntityTypeId = EntityTypeCache.Get ( Rock.SystemGuid.EntityType.GROUP_MEMBER ).Id;
                            newAttribute.EntityTypeQualifierColumn = "GroupTypeId";
                            newAttribute.EntityTypeQualifierValue = newGroup.GroupTypeId.ToString ();
                            newAttribute.FieldTypeId = _groupFieldTypeId;
                    ...
                    rockContext.Database.CommandTimeout = 900; // 15 minutes (usually takes 25 seconds to do 500 people in one group)

                    WriteProgressMessage ( "Step 3 of 5 in progress...", "Updating existing people in groups..." );
        ...
        protected void btnUndo_Click( object sender, EventArgs e )
        {
            ...
            var importTask = new Task ( () =>
            {
                ...
                if (qryMemberValues.Any())
                {
                    ...
                    // Add members to New Groups
                    WriteProgressMessage ( "Re-Adding to Old Groups...", "Updating current members in old groups." );
        ...
        private void BindGrid()
        {
            ...
            var attributeValueService = new AttributeValueService ( rockContext );
            var promotesToId = AttributeCache.Get (_groupAttributeGuid ).Id;
            ...
            var qryValues = attributeValueService.Queryable ()
                .Where ( av => av.AttributeId == promotesToId && av.Value != "" )
                .Select ( av => av.EntityId )
                .ToList ();
                ...
                .Select ( g => new GroupListRowInfo
                {
                    ...
                    PromotesTo = new Func<String> ( () =>
                    {
                        ...
                        if ( gr != null )
                        {
                            ...
                            var pg = groupService.Get ( gr.GetAttributeValue ( AttributeCache.Get ( _groupAttributeGuid ).Key ).AsGuid () );
                            if (pg != null)
                            {
                                return pg.Name;
                            }
                            ...
                            {
                                return "ERROR - promotion group not valid.";
                            }
                        }
                        return String.Empty;
                    } ) ()

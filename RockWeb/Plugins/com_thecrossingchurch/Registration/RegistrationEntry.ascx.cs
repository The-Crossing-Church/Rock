    ...
    public partial class RegistrationEntry : RockBlock
    {
        ...
        protected void lbSummaryNext_Click( object sender, EventArgs e )
        {
            if ( CurrentPanel == PanelIndex.PanelSummary )
            {
                ...
                {
                    if ( summaryErrors.Any(se => se == "This event has reached capacity") )
                    {
                        ShowWarning(
                            string.Format("{0} Full", RegistrationTerm),
                            string.Format("<p>There are not any more {0} available for {1}.</p>", RegistrationTerm.ToLower().Pluralize(), RegistrationInstanceState.Name));
                        pnlSummaryAndPayment.Visible = false;
                    }
                    else
                    {
        ...
        private List<string> ValidateSummary()
        {
            ...
            //Check that number of people registered is still valid
            if ( RegistrationState.SlotsAvailable.HasValue )
            {
                var registrations = new RegistrationService(new RockContext()).Queryable().Where(r => r.RegistrationInstanceId == RegistrationInstanceState.Id).ToList();
                var registrants = registrations.Select(r => r.Registrants.Count());

                RegistrationState.SlotsAvailable = RegistrationInstanceState.MaxAttendees - registrants.Sum();
                if ( RegistrationState.Registrants.Count() > RegistrationState.SlotsAvailable.Value )
                {
                    validationErrors.Add("This event has reached capacity");
                }
            }

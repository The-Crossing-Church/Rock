...
{
	/// <remarks>
	/// Copy of v9.2 block with the addition of being able to view/edit attributes associated with a business.
	/// </remarks>
    [DisplayName( "Business Detail" )]
    [Category( "Pillars > Finance" )]
    [Description( "Displays the details of the given business (with attributes)." )]
    ...
    public partial class BusinessDetail : Rock.Web.UI.RockBlock, IDetailBlock
    {
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            var businessDvId = DefinedValueCache.Get( Rock.SystemGuid.DefinedValue.PERSON_RECORD_TYPE_BUSINESS.AsGuid() ).Id;
            var business = new PersonService( new RockContext() ).Get( businessId );
            if ( business == null )
            {
                business = new Person
                {
                    RecordTypeValueId = businessDvId
                };
            }
            business.LoadAttributes();
            string qualifier = businessDvId.ToString();
            var excludedAttributes = business.Attributes
                .Where( a => 
                    !( a.Value.EntityTypeQualifierColumn == "RecordTypeValueId" && a.Value.EntityTypeQualifierValue == qualifier ) )
                .Select( a => a.Value.Key )
                .ToList();

            phAttributes.Controls.Clear();
            Helper.AddEditControls( business, phAttributes, true, BlockValidationGroup, excludedAttributes );
        }
        ...
        protected void lbSave_Click( object sender, EventArgs e )
        {
            ...
            Rock.Attribute.Helper.GetEditValues( phAttributes, business );
        ...
        private void ShowSummary( int businessId )
        {
            ...
            if ( business != null )
            {
                ...
                business.LoadAttributes();
                string qualifier = DefinedValueCache.Get( Rock.SystemGuid.DefinedValue.PERSON_RECORD_TYPE_BUSINESS.AsGuid() ).Id.ToString();
                foreach( var attribute in business.Attributes
                    .Where( a =>
                        ( a.Value.EntityTypeQualifierColumn == "RecordTypeValueId" && a.Value.EntityTypeQualifierValue == qualifier ) &&
                        a.Value.IsGridColumn )
                    .Select( a => a.Value ) )
                {
                    detailsLeft.Add( attribute.Name, attribute.FieldType.Field.FormatValue( null, business.GetAttributeValue( attribute.Key ), attribute.QualifierValues, false ) );
                }

...
namespace RockWeb.Plugins.com_scottsdalebible.Finance
{
    /// <summary>
    /// 
    /// </summary>
    [DisplayName( "Transaction Matching" )]
    [Category( "Scottsdale Bible > Finance" )]
    ...
    [AttributeField(
		...
		"Note Attribute", 
		"An optional person attribute that should be displayed as an editable note.", 
		false, 
		false, 
		"", 
		"", 
    ...
    [AttributeField(
        Rock.SystemGuid.EntityType.PERSON,
        "Default Soft Credit Attribute",
        "An optional person attribute that defines another person that contributions should be a soft credit for..",
        false,
        false,
        "",
        "",
...
", btnNext.ClientID, ddlAddAccount.ClientID );
        ...
        private void NavigateToTransaction( Direction direction )
        {
            ...
            lock ( transactionMatchingLockObject )
            {
                ...
                if ( transactionToMatch != null )
                {
                    ...
                    tbNewMicr.Text = string.Empty;

                    string decryptedMicr = string.Empty;
                    try
                    {
                        tbNewMicr.Text = Rock.Security.Encryption.DecryptString( transactionToMatch.CheckMicrEncrypted );
                    }
                    catch { }
                    ...
                    var softCreditsFor = new List<string>();
                    foreach( var dtl in transactionToMatch.TransactionDetails )
                    {
                        dtl.LoadAttributes();
                        softCreditsFor.Add( dtl.GetAttributeValue( "SoftCreditFor" ) );
                    }
                    softCreditsFor = softCreditsFor.Distinct().ToList();
                    if ( softCreditsFor.Count() <= 1 )
                    {
                        Person softCreditPerson = null;

                        if ( softCreditsFor.Count == 1 )
                        {
                            Guid? paGuid = softCreditsFor.First().AsGuidOrNull();
                            if ( paGuid.HasValue )
                            {
                                softCreditPerson = new PersonAliasService( rockContext ).GetPerson( paGuid.Value );
                            }
                        }

                        ppSoftCredit.Visible = true;
                        ppSoftCredit.SetValue( softCreditPerson );
                    }
                    else
                    {
                        ppSoftCredit.Visible = false;
                    }

                    Person authorizedPerson = transactionToMatch.AuthorizedPersonAlias != null ? transactionToMatch.AuthorizedPersonAlias.Person : null;

                    SetMicrDisplay();
                    SetPerson( checkMicrHashed, authorizedPerson, rockContext );
        ...
        private void SetMicrDisplay()
        {
            lMicrData.Text = string.Empty;
            var errMsg = string.Empty;

            if ( tbNewMicr.Text.IsNotNullOrWhiteSpace() )
            {
                Regex validMicrRegEx = new Regex( @"^[-0-9dbc ]+$" );
                var validMicr = validMicrRegEx.Match( tbNewMicr.Text );
                if ( !validMicr.Success )
                {
                    errMsg = "Invalid MICR";
                }

                var micr = new com.shepherdchurch.ImageCashLetter.Micr( tbNewMicr.Text );
                var rt = micr.GetRoutingNumber();
                var acct = micr.GetAccountNumber();
                var item = micr.GetCheckNumber();

                if ( rt.Length < 9 && errMsg == string.Empty )
                {
                    errMsg = "Invalid Routing Number";
                }

                lMicrData.Text = string.Format( @"
<span class='margin-r-md'><strong>Routing# </strong>{0}</span>
<span class='margin-r-md'><strong>Account# </strong>{1}</span>
<span class='margin-r-md'><strong>Item# </strong>{2}</span>
", rt, acct, item );

                if ( errMsg != string.Empty )
                {
                    lMicrData.Text = string.Format( "<div>{0}<br/><i>{1}</i></div>", lMicrData.Text, errMsg );
                    pnlMicrView.AddCssClass( "alert" );
                    pnlMicrView.AddCssClass( "alert-danger" );
                }
                else
                {
                    pnlMicrView.RemoveCssClass( "alert" );
                    pnlMicrView.RemoveCssClass( "alert-danger" );
                }
        ...
        private void SetPerson( string checkMicrHashed, Person authorizedPerson, RockContext rockContext )
        {
            ...
            {
                var matchedPersons = new FinancialPersonBankAccountService( rockContext ).Queryable().Where( a => a.AccountNumberSecured == checkMicrHashed ).Select( a => a.PersonAlias.Person ).Distinct();
            ...
            { 
                ...
                if ( !ddlIndividual.Items.OfType<ListItem>().Any( a => a.Value == authorizedPerson.Id.ToString() ) )
                {
                    ddlIndividual.Items.Add( new ListItem( authorizedPerson.FullNameReversed, authorizedPerson.Id.ToString() ) );
                }

                ddlIndividual.SelectedValue = authorizedPerson.Id.ToString();
            }
        ...
        private void ShowPreviousTransactions()
        {
            ...
            if ( authorizedPersonId.HasValue )
            {
                ...
                var rockContext = new RockContext();
                var person = new PersonService( rockContext ).Get( authorizedPersonId.Value );
                if ( person != null )
                {
                    ...
                    txns = new FinancialTransactionService( rockContext )
                        .Queryable().AsNoTracking()
                        .Where( t =>
                            t.TransactionDateTime.HasValue &&
                            t.TransactionTypeValue != null &&
            ...
            gTransactions.DataSource = txns;
            gTransactions.DataBind();
        }

        private void ShowPledges()
        {
            var pledges = new List<FinancialPledge>();
            ...
            if ( authorizedPersonId.HasValue )
            {
                ...
                if ( person != null )
                {
                    ...
                    foreach( var pledge in new FinancialPledgeService( rockContext )
                        .Queryable().AsNoTracking()
                        .Where( p =>
                            p.PersonAliasId.HasValue &&
                            personAliasIds.Contains( p.PersonAliasId.Value ) &&
                            p.StartDate <= today &&
                            p.EndDate >= today )
                        ...
                        .ToList() )
                    {
                        ...
                        var dvStatus = DefinedValueCache.Get( pledge.GetAttributeValue( "Status" ).AsGuid() );
                        if ( dvStatus.Value == "Active")
                        {
                            pledges.Add( pledge );
                        }
                    }
        ...
        protected void btnFilter_Click( object sender, EventArgs e )
        {
            ...
            ShowDialog( "AccountsFilter", true );
        }
            ...
            if ( financialTransaction != null && authorizedPersonId.HasValue )
            {
                ...
                foreach ( var accountBox in rptAccounts.ControlsOfTypeRecursive<CurrencyBox>() )
                {
                    ...
                    if ( amount.HasValue && amount.Value >= 0 )
                    {
                        ...
                        details.Add( financialTransactionDetail );
                ...
                if ( ppSoftCredit.Visible )
                {
                    string value = string.Empty;
                    if ( ppSoftCredit.PersonId.HasValue )
                    {
                        var softCreditPersonAlias = new PersonAliasService( rockContext ).GetPrimaryAlias( ppSoftCredit.PersonId.Value );
                        value = softCreditPersonAlias != null ? softCreditPersonAlias.Guid.ToString() : string.Empty;
                    }

                    foreach ( var dtl in details )
                    {
                        dtl.LoadAttributes();
                        dtl.SetAttributeValue( "SoftCreditFor", value );
                        dtl.SaveAttributeValue( "SoftCreditFor", rockContext );
                    }
                }				
            }
            else
        ...
        private void LoadPersonPreview( int? personId )
        {
            ...
            pnlNoteView.Visible = false;
            pnlNoteEdit.Visible = false;
            ...
            if ( person != null )
            {
                ...
                if ( spouse != null )
                {
                    hfSpouseId.Value = spouse.Id.ToString();
                    phSpouseName.Visible = true;
                    btnSpouseName.Text = spouse.FullName;
                }
                else
                {
                    hfSpouseId.Value = string.Empty;
                    phSpouseName.Visible = false;
                }
                ...
                Guid? guid = GetAttributeValue( "NoteAttribute" ).AsGuidOrNull();
                if ( guid.HasValue )
                {
                    var attribute = AttributeCache.Get( guid.Value, rockContext );
                    if ( attribute != null )
                    {
                        lNote.Label = attribute.Name;
                        tbNote.Label = attribute.Name;
                        lNote.Text = person.GetAttributeValue( attribute.Key );
                        tbNote.Text = lNote.Text;

                        ShowNoteView();
                    }
                }

                if ( ppSoftCredit.Visible && !ppSoftCredit.SelectedValue.HasValue )
                {
                    guid = GetAttributeValue( "DefaultSoftCreditAttribute" ).AsGuidOrNull();
                    if ( guid.HasValue )
                    {
                        var attribute = AttributeCache.Get( guid.Value, rockContext );
                        if ( attribute != null )
                        {
                            var paGuid = person.GetAttributeValue( attribute.Key ).AsGuidOrNull();
                            if ( paGuid.HasValue )
                            {
                                var scPerson = new PersonAliasService( rockContext ).GetPerson( paGuid.Value );
                                if ( scPerson != null )
                                {
                                    ppSoftCredit.SetValue( scPerson );
                                }
                            }
                        }
                    }
        ...
        protected void ShowNoteView()
        {
            pnlNoteEdit.Visible = false;
            pnlNoteView.Visible = true;

            if ( lNote.Text.IsNotNullOrWhiteSpace() )
            {
                pnlNoteView.AddCssClass( "alert" );
                pnlNoteView.AddCssClass( "alert-info" );
            }
            else
            {
                pnlNoteView.RemoveCssClass( "alert" );
                pnlNoteView.RemoveCssClass( "alert-info" );
            }
        }
        ...
        protected void lbEditMicr_Click( object sender, EventArgs e )
        {
            rptrImages.Visible = false;
            pnlMicrEdit.Visible = true;
            pnlMicrView.Visible = false;
        }

        protected void lbSaveMicr_Click( object sender, EventArgs e )
        {
            int txnId = hfTransactionId.Value.AsInteger();
            ...
            if ( financialTransaction != null )
            {
                ...
                if ( micr != null )
                {
                    string rt = micr.GetRoutingNumber();
                    string acct = micr.GetAccountNumber();
                    string item = micr.GetCheckNumber();

                    financialTransaction.CheckMicrEncrypted = Encryption.EncryptString( tbNewMicr.Text );
                    financialTransaction.CheckMicrHash = Encryption.GetSHA1Hash( tbNewMicr.Text );
                    financialTransaction.CheckMicrParts = Encryption.EncryptString( string.Format( "{0}_{1}_{2}", rt, acct, item ) );
                    rockContext.SaveChanges();

                    SetMicrDisplay();

                    Person authorizedPerson = financialTransaction.AuthorizedPersonAlias != null ? financialTransaction.AuthorizedPersonAlias.Person : null;
                    string checkMicrHashed = FinancialPersonBankAccount.EncodeAccountNumber( rt, acct );
                    SetPerson( checkMicrHashed, authorizedPerson, rockContext );
                }
            }

            rptrImages.Visible = true;
            pnlMicrEdit.Visible = false;
            pnlMicrView.Visible = true;
        }

        protected void lbCancelMicr_Click( object sender, EventArgs e )
        {
            rptrImages.Visible = true;
            pnlMicrEdit.Visible = false;
            pnlMicrView.Visible = true;
        ...
        protected void lbEditNote_Click( object sender, EventArgs e )
        {
            pnlNoteEdit.Visible = true;
            pnlNoteView.Visible = false;
        }
            ...
            if ( guid.HasValue )
            {
                ...
                if ( authorizedPersonId.HasValue )
                {
                    ...
                    if ( person != null )
                    {
                        ...
                        if ( attribute != null )
                        {
                            Rock.Attribute.Helper.SaveAttributeValue( person, attribute, tbNote.Text, rockContext );
                            lNote.Text = tbNote.Text;
                        }
                    }
                }
            }
        ...
        protected void lbCancelNote_Click( object sender, EventArgs e )
        {
            tbNote.Text = lNote.Text;
            ShowNoteView();
        }

        protected void btnSpouseName_Click( object sender, EventArgs e )
        {
            int? personId = hfSpouseId.Value.AsIntegerOrNull();
            if ( personId.HasValue )
            {
                var rockContext = new RockContext();
                ...
                if ( person != null )
                {
                    SetPerson( hfCheckMicrHashed.Value, person, rockContext );
                }
            }
        }
        ...
        {
            ShowDialog( "PreviousTransactions", true );
        }

        protected void btnPledges_Click( object sender, EventArgs e )
        {
            ShowDialog( "Pledges", true );
        }

        protected string GetAccountDetails( object obj )
        {
            var txn = obj as FinancialTransaction;
            if ( txn != null )
            {
                List<string> summary = null;
                summary = txn.TransactionDetails
                    .Select( d => new
                    {
                        d.Account.Order,
                        d.Account.Name,
                    ...
                    .OrderBy( d => d.Order )
                    .Select( d => string.Format( "{0}: {1}", d.Name, d.Amount.FormatAsCurrency() ) )
                    .ToList();

                if ( summary != null && summary.Any() )
                {
                    return "<small>" + summary.AsDelimited( ", " ) + "</small>";
                }
            }

...
{
    [DisplayName( "Report Detail With Lava" )]
    [Category("com_bemadev > Reporting")]
    ...
    [LinkedPage("Print Action Page", "Page for displaying the report in a printable format.  Navigate to this page when print is clicked.", false, order:3)]
    [BooleanField( "Use Printable Format", "This removes controls around the grid and just gives the basic grid in a printable format.", false, order: 4 )]
    [CodeEditorField("Formatted Output", "Optional formatting to apply to the returned results.  If left blank, a grid will be displayed. Example: {% for row in rows %} {{ row.FirstName }}<br/> {% endfor %}",
        CodeEditorMode.Lava, CodeEditorTheme.Rock, 200, false, "", "")]

    public partial class ReportDetailWithPDF : RockBlock, IDetailBlock
    {
        #region Properties
        ...
        private LinkButton _lbGeneratePDF;
        private Literal _lTitle = new Literal();
        ...
        protected override void OnLoad( EventArgs e )
        {
            ...
            // handle PDF printable page if enabled
            var printActionPage = GetAttributeValue("PrintActionPage");
            bool usePrintableFormat = GetAttributeValue("UsePrintableFormat").AsBoolean();
            bool? overridePrintableFormat = PageParameter("usePrintableFormat").AsBooleanOrNull();
            if (overridePrintableFormat.HasValue)
            {
                usePrintableFormat = overridePrintableFormat.Value;
            }

            if (!string.IsNullOrWhiteSpace(printActionPage) && !usePrintableFormat)
            {
                var lbPrintBtn = new LinkButton();
                gReport.Actions.Controls.Add(lbPrintBtn);
                lbPrintBtn.ID = "lbPrintBtn";
                lbPrintBtn.CssClass = "btn btn-default btn-sm";
                lbPrintBtn.ToolTip = "Printable PDF";
                lbPrintBtn.Click += lbPrintBtn_Click;
                lbPrintBtn.CausesValidation = false;
                lbPrintBtn.PreRender += lb_PreRender;
                gReport.Actions.Controls.Add(lbPrintBtn);
                HtmlGenericControl iPrint = new HtmlGenericControl("i");
                iPrint.Attributes.Add("class", "fa fa-print fa-fw");
                lbPrintBtn.Controls.Add(iPrint);
            }
        ...
        void lbPrintBtn_Click(object sender, EventArgs e)
        {
            ...
            var page = GetAttributeValue("PrintActionPage").AsGuidOrNull();
            Rock.Model.Page rockPage = null;
            if (page.HasValue)
            {
                rockPage = pageService.Get(page.Value);
            }

            int? reportId = PageParameter("ReportId").AsIntegerOrNull();
            if (!reportId.HasValue)
            {
                reportId = reportGuid != null ? new ReportService(new RockContext()).GetId(reportGuid.Value) : null;
            }

            var url = string.Format("~/page/{0}?UsePrintableFormat=1&ReportId={1}",
                rockPage.Id, reportId.Value);
        ...
        void lb_PreRender(object sender, EventArgs e)
        {
            ...
            if (lb != null)
            {
                lb.Enabled = gReport.Actions.Enabled;
                if (lb.Enabled)
                {
        ...
        private void BindGrid( Report report, bool isCommunication )
        {
            ...
            if (usePrintableFormat || !formattedOutput.IsNullOrWhiteSpace())
            {
                gReport.AllowPaging = false;
                gReport.AllowCustomPaging = false;
            }

            var filterJson = PageParameter( "FilterJson" );
            if ( filterJson.IsNotNullOrWhiteSpace() )
            {
                var dataViewFilterOverrides = JsonConvert.DeserializeObject<DataViewFilterOverrides>(System.Web.HttpUtility.UrlDecode(filterJson));
            ...
            if (!formattedOutput.IsNullOrWhiteSpace())
            {
                ...
                foreach ( DataControlField col in gReport.Columns )
                {
                    dataset.Tables[0].Columns.Add( col.ToString().Replace(" ", "") );
                }

                foreach (GridViewRow row in gReport.Rows)
                {
                    object[] rowVals = new object[dataset.Tables[0].Columns.Count];
                    int col = 0;
                    foreach ( TableCell cell in row.Cells )
                    {
                        rowVals[col++] = cell.Text;
                    }
                    dataset.Tables[0].Rows.Add( rowVals );
                }
                ...
                lReport.Text = formattedOutput.ResolveMergeFields( mergeFields );
                lReport.Visible = true;
                gReport.Visible = false;
            }
            // use printable grid that can be printed to PDF
            else if (usePrintableFormat)
            {
                ...
                gReport.ShowHeader = false;
                gReport.ShowFooter = false;
                //gReport.BottomPagerRow.Visible = false;
                gReport.IsDeleteEnabled = false;
                gReport.RowClickEnabled = false;
                gReport.Columns[0].Visible = false;
                btnToggleResults.Visible = false;

                pnlHeading.Visible = false;

                gReport.ActionRow.Visible = false;
                gReport.ShowActionsInHeader = true;
                gReport.Actions.Visible = true;
                gReport.Actions.ShowBulkUpdate = false;
                gReport.Actions.ShowCommunicate = false;
                gReport.Actions.ShowExcelExport = false;
                gReport.Actions.ShowMergePerson = false;
                gReport.Actions.ShowMergeTemplate = false;
                gReport.Actions.ClearCustomActionControls();

                _lTitle.Text = string.Format("<h5><center>{0}</center></h5>", report.Name);
                _lTitle.Visible = true;
                gReport.Actions.Controls.Add(_lTitle);

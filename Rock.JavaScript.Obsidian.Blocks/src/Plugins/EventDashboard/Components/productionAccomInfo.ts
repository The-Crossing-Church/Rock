...
export default defineComponent({
    ...
    computed: {
      ...
      sectionIsValid() {
        if(this.requestValidation?.invalidSections) {
          if(this.requestValidation.invalidSections.includes("Production")) {
            return false
          } else {
            return true
          }
      ...
      errors() {
        ...
        if(!this.sectionIsValid) {
          if(this.requestValidation?.errors && this.requestValidation.errors.length > 0) {
            let item_errors = this.requestValidation.errors.filter(err => err.ref == "prodtech")
    ...
    template: `
<div>
  <h4 class="text-accent">Production Accomodation Information</h4>
  <div class="row">

...
export default defineComponent({
    name: "EventDashboard.Components.Modal.OffsiteInfo",
    components: {
      "rck-field": RockField
    },
    props: {
    ...
    computed: {
      offSiteAttrs() {
        ...
        if(this.details?.attributes && this.details.attributeValues) {
          for(let key in this.details.attributes) {
            ...
            if(categories.includes("Event Offsite")) {
      ...
      sectionIsValid() {
        if(this.offSiteAttrs) {
          let attrsWithErrors = this.offSiteAttrs.filter((i: any) => i.errors.length > 0)
          if(attrsWithErrors && attrsWithErrors.length > 0) {
            return false
          }
        }
        return true
...
<div>
  ...
  <div class="row">
    <div class="col col-xs-12 col-md-6" v-for="av in offSiteAttrs">

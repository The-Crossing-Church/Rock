...
export default defineComponent({
  ...
  setup() {
      ...
      const userAction: (id: string | null | undefined, actin: string) => Promise<any> = async (id, action) => {
...
<div class="card">
  ...
  <a-table :columns="columns" :data-source="viewModel.events" :pagination="{ pageSize: 30 }">
    <template #bodyCell="{ column, record }">
      ...
      <template v-else-if="column.key == 'action'">
        <tcc-grid :request="record" :commentNotification="lastCommentIsFromUser(record)" v-on:duplicate="copyFromGridAction" v-on:updatestatus="updateFromGridAction"></tcc-grid>
      </template>
    </template>
  </a-table>
  <a-modal v-model:visible="modal" width="80%" :closable="false">
    ...
    <template #footer>
      <div class="text-left">
        <a-btn v-if="canEdit" type="primary" @click="editItem(selected.idKey)">
        ...
        <a-btn type="primary" v-if="selectedStatus == 'Proposed Changes Denied'" @click="proposedChangesAction('Original', selected.idKey)">

...
namespace org.crossingchurch.GroupSchedulerFollowUp.Jobs
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public class GroupSchedulerFollowUp : IJob
    {
        ...
        public virtual void Execute( IJobExecutionContext context )
        {
            ...
            List<Person> noResponsePeople = new List<Person>();
            ...
            for ( var i = 0; i < groups.Count(); i++ )
            {
                ...
                {
                    var hasResponded = HasResponded(groups[i], members[j].Person);
                    if ( !hasResponded && !noResponsePeople.Select(p => p.Id).Contains(members[j].Person.Id) )
                    {
                        noResponsePeople.Add(members[j].Person);
                    }
                }
            }
            //Send Notifications to Scheduled People
            for ( var i = 0; i < noResponsePeople.Count(); i++ )
            {
                SendMessage(noResponsePeople[i]);
            }
        }
        ...
        private bool HasResponded( Group group, Person person )
        {
            //Check to see if Person has responded to invitations
            ...
            for ( var i = 0; i < locations.Count(); i++ )
            {
                ...
                for ( var j = 0; j < schedules.Count(); j++ )
                {
                    ...
                    var attendance = AttendanceSvc.Get(nextOcc.Value, locations[i].LocationId, schedules[j].Id, group.Id, person.Id);
                    if ( attendance.RSVP == RSVP.Yes || attendance.RSVP == RSVP.No ) {
                        return true;
                    }
                }
            }

        ...
        public virtual void Execute( IJobExecutionContext context )
        {
            ...
            for ( var i = 0; i < contacts.Count(); i++ )
            {
                ...
                if ( person == null )
                {
                    ...
                    {
                        var phone_matches = personService.Queryable().Where( p => p.PhoneNumbers.Any( ph => contact.properties.phone.Contains( ph.Number ) || ph.Number.Contains( contact.properties.phone ) ) ).ToList();
